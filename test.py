...
def file():
            ...
            print("\033[1;92m《𝟮》𝙲𝚛𝚎𝚊𝚝𝚎 𝙵𝚒𝚕𝚎")
            print("\033[1;92m《𝟯》𝚂𝚘𝚌𝚒𝚊𝚕 𝙼𝚎𝚍𝚒𝚊 ")
            key = input("\n《➙》𝐒𝐄𝐋𝐋𝐄𝐂𝐓 : ")
            if key in [""]:
...
def alok():
    try:
        ...
        os.system("clear")
        logo()
        print(' [%s!%s] This Tool Are Paid Contact Admin for Buy Tool     %s350 • 15 days / 750 • 30 days %s'%(M,N,H,N))
        print('\n %s%sMenu Tools%s'%(BM,P,N))
        print(' [%s1%s] Login Key Tools'%(H,N))
        print(' [%s2%s] Contact Admin'%(H,N))
        pil = input('\n %s[%s?%s] Choice : '%(N,K,N))
        if pil =="":
            jalan(f" {N}[{M}×{N}] 𝙸𝚗𝚌𝚘 𝚄𝚜𝚎𝚛/𝙿𝚊𝚜𝚜 𝙿𝚕𝚎𝚊𝚜𝚎 𝙻𝚘𝚐𝚒𝚗 𝙵𝚒𝚛𝚜𝚝...!");time.sleep(1);alok()
        elif pil in["2","02"]:
            ...
            exit(f"{N}[{M}×{N}] 𝙸𝚗𝚌𝚘 𝚄𝚜𝚎𝚛/𝙿𝚊𝚜𝚜 𝙿𝚕𝚎𝚊𝚜𝚎 𝙻𝚘𝚐𝚒𝚗 𝙵𝚒𝚛𝚜𝚝...!")
