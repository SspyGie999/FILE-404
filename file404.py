...
def hasil(ok,cp):
    if len(ok) != 0 or len(cp) != 0:
        ...
        if cek_cp =="":
            ...
            ww=input(f"\n {N}[{K}?{N}] Change password when {BM}TAP YES{N} [{H}Y{N}/{M}N{N}]: ")
            if ww in ("Y","y","ya"):
                ubahP.append("y")
                print(f" {N}[{H}•{N}] Password example : {H}file404123{N}")
            ...
            for memek in cp:
                ...
                titid  = kontol.split(' • ')
                print(f'{N}══════════════════════════════════════════\n {H}LOGIN PROCESS')
                jalan(f' {N}[{M}?{N}] Account : {K}{kontol.replace("[404-LOCKED] ", "")}{N}')
                try:
                    log_hasil(titid[0].replace("[404-LOCKED] ", ""), titid[1])
...
def cek_apk(session,coki):
    ...
    if len(game)==0:
        ...
        print(f'\r [ 🕹️ ]%s Active Games Details :'%(H))
    ...
    if len(game)==0:
        ...
        print(f'\r [ 🕹️ ]%s Expired App Details :'%(M))
        for i in range(len(game)):
...
def file():
            ...
            print ("\033[1;92m[2] Check Return ")
            key = input("\n[+] Select : ")
            if key in [""]:
...
class __chigoue__:
    ...
    def chi(self, id):
        ...
        logo()
        print ('\x1b[1;92m══════════════════════════════════════════════════════')
        crot = input(f"{H}[{H}+{H}] do you want to see the app? [{H}Y{H}/{H}N{H}]: ")
        if crot in[""]:
        ...
        self.cnt = input('\033[1;92m[+] Enter File Name :\x1b[1;91m ')
...
# PROSES CRACK METODE 3 in 1
    def __metode__(self, cebok, user, pasw):
        global ok,cp,loop
        animasi = random.choice(["\x1b[1;92m[ Starting ]","\x1b[1;92m[ Starting ]","\x1b[1;92m[ Starting ]","\x1b[1;92m[ Starting ]","\x1b[1;92m[ Starting ]","\x1b[1;92m[ Starting ]","\x1b[1;92m[ Starting ]"])
    ...
    def __pler__(self):
        ...
        logo()
        print ('\x1b[1;92m══════════════════════════════════════════════════════')
        print (' Select Your Best Method in 1/3')
        print ('\x1b[1;92m══════════════════════════════════════════════════════')
        print ('[1] Free Facebook')
        print ('[2] M.basic Facebook')
        print ('[3] M.facebook')
    ...
    def kombinasi_pw(self,url):
        print('%s══════════════════════════════════════════════════════\n%sSetting Password Menu%s\n══════════════════════════════════════════════════════'%(H,H,H))
        print('%s[%s1%s] Auto Password 1'%(H,H,H))
        print('%s[%s2%s] Auto Password 2'%(H,H,H))
...
def alok():
    try:
        ...
        logo()
        print('\x1b[1;92m══════════════════════════════════════════════════════')
        print('[%s!%s] Contact Admin if anyone is buy and selling SC Meta. %s FILE-404 %s'%(M,N,H,N))
        print('\x1b[1;92m══════════════════════════════════════════════════════')
        print('\n %s%s              Menu Tools%s'%(BM,P,N))
        print('\x1b[1;92m══════════════════════════════════════════════════════')
        ...
        if pil =="":
            ...
            os.system('xdg-open https://www.facebook.com/regiemontemayor050922i');time.sleep(2);alok()
        elif pil in["1","01"]:
            print('%s══════════════════════════════════════════════════════'%(N))
